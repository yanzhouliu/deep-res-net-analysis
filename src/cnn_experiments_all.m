...
tr_file_h = 'cifar10_L127';
te_file_h = 'cifar10_L127';
...
idx = [2 3 4 6 7 8 10 11 12];
size_idx = size(idx,2);
net_tr_error=zeros(size_idx, size_h2, size_f1, numEpochs);
net_val_error=zeros(size_idx, size_h2, size_f1, numEpochs);
numEpochs = 50;
...
for i = 1:size_idx
    ...
    h_str = sprintf('data/d%d_h1_%dh2_%df1_%df2_%d', idx(i), h1, h2, f1, f2);
    tr_file = sprintf('%s_D%d_10_train.mat');
    te_file = sprintf('%s_D%d_10_test.mat');
    ...
    fig_filename1 = sprintf('data/d%dh1_%dh2_%df1_%df2_%d.fig', idx(i), h1, h2, f1, f2);
    fig_filename2 = sprintf('data/d%dh1_%dh2_%df1_%df2_%d.png', idx(i), h1, h2, f1, f2);
    ...
    %change
    tmp = size(value2,1);
    net_tr_error(i, 1, 1, 1:tmp)=value2;

    value = getfield(info_bn, 'val');
    value2 = arrayfun(@(x) x.top1err, value);
    value2 = value2';
    %change
    net_val_error(i,1, 1, 1:tmp)=value2;
